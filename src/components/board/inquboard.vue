  ...
  setup() {
    ...
    const handleData = async () => {
      ...
      if (state.pagecnttable.value == undefined) {
        const url = `/TEAMMOVIE/api/board/selectlist?page=${state.page}&title=${state.title}&pagecnt=5&type=1`;
        const headers = {
        ...
        const url = `/TEAMMOVIE/api/board/selectlist?page=${state.page}&title=${state.title}&pagecnt=${state.pagecnttable.value}&type=1`;
        const headers = {
          "Content-Type": "application/json",
        };
        const response = await axios.get(url, { headers });

  ...
  <div>
    <div style="min-height: 520px">
      ...
      >
        ...
        <el-table-column prop="btitle" label="제목" width="180" />
        <!-- <el-table-column prop="btype" label="타입" width="180" /> -->
        <el-table-column prop="memberEntityMid" label="작성자" width="180" />
        <el-table-column prop="bhit" label="조회수" width="180" />
  ...
  setup() {
    ...
    const handleData = async () => {
      ...
      if (state.pagecnttable.value == undefined) {
        const url = `/TEAMMOVIE/api/board/selectlist?page=${state.page}&title=${state.title}&pagecnt=5&type=0`;
        const headers = {
        ...
        const url = `/TEAMMOVIE/api/board/selectlist?page=${state.page}&title=${state.title}&pagecnt=${state.pagecnttable.value}&type=0`;
        const headers = {
          "Content-Type": "application/json",
        };
        const response = await axios.get(url, { headers });
